export const site = {
  name: 'Star Gate Tourism & Travel LLC',
  shortName: 'Star Gate',
  tagline: 'Explore the World With Us',
  founded: 2019,
  location: 'Dubai & Sharjah, UAE',
  accreditation: 'IATA Accredited Agent',
  corePromise:
    'Premium visa processing, luxury travel packages, and unforgettable experiences — crafted for the discerning traveler.',
  phoneDisplay: '+971 54 993 6771',
  phoneHref: 'tel:+971549936771',
  whatsappDisplay: '+971 54 993 6771',
  whatsappHref: 'https://wa.me/971549936771',
  email: 'dxb@stargatetours.ae',
  address: 'Shop No. 76A, Nasser Loothah Building, Al Tawar 1 (near DAFZA Metro Station), Dubai, UAE',
}

export const branches = [
  {
    name: 'Al Qusais (Head Office)',
    address: 'Shop No. 76A, Nasser Loothah Building, Al Tawar 1 (near DAFZA Metro Station), Dubai, UAE',
    hours: 'Monday–Saturday, 9:30 AM – 6:30 PM',
    opened: 2019,
    mapQuery: 'Star Gate Tourism & Travel LLC, Al Qusais, Dubai',
  },
  {
    name: 'Lulu Village, Muhaisnah 4',
    address: 'Lulu Village, Muhaisnah 4, Dubai, UAE',
    hours: 'Every day, 10:00 AM – 11:00 PM',
    opened: 2021,
    mapQuery: 'Star Gate Tourism & Travel, Lulu Village, Muhaisnah 4, Dubai',
  },
  {
    name: 'Muwailah, Sharjah',
    address: 'Muwailah, Sharjah, UAE',
    hours: 'Mon–Sat, 9:00 AM – 10:00 PM · Sunday, 4:00 PM – 10:00 PM',
    opened: 2023,
    mapQuery: 'Star Gate Tourism LLC, Muwailah, Sharjah',
  },
] as const

export const sisterCompanies = [
  {
    tabLabel: 'Prime Point',
    name: 'Prime Point Documents Clearing LLC',
    address: 'First Floor, Office No. 103, Montana Building, Zabeel Street, Al Karama, Dubai, UAE',
    phones: [
      { display: '+971 54 993 6771', href: 'tel:+971549936771' },
      { display: '+971 56 681 7584', href: 'tel:+971566817584' },
    ],
    mapQuery: '25.249485,55.309370',
  },
] as const

export const SERVICE_NAMES = [
  'UAE Tourist Visa',
  'Visa Change By Bus',
  'Visa Change By Flight',
  'Air Tickets',
  'Travel Insurance',
  'Holiday Package',
] as const
